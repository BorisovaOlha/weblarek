...
Методы класса:
`setData(data: IBuyer): void` - сохраняет данные покупателя, переданные в параметрах.
`getData(): IBuyer` - возвращает все данные покупателя.
`emptyData(): void` - очищает данные покупателя.
`checkData(data: IBuyer): object` - проверяет данные покупателя и возвращает результат проверки.
